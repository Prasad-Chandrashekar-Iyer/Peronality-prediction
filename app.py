...
UPLOAD_FOLDER = 'C:/Users/ccpda/Desktop/Aipro/CV/'
...
def sendmaila():
    ...
    command2= "select stud_email from student_profile where stud_id=%s"
    cursor.execute(command)
    res = cursor.fetchall()
    cursor.execute(command2,uid)
    email= cursor.fetchone()
    gmail_user = 'iyer1998@gmail.com'
    gmail_password = 'resyeuzwzqeezqgg'
    print("fdgfdgfdg",email)
    sent_from = gmail_user
    to = [email[0]]
    subject = 'from project' #Line that causes trouble
    msg = 'Subject:{}\n\n This is test message from server'.format(subject)
...
def prediction():
    if request.method == "POST":
        connection.ping(reconnect=True)
        cursor = connection.cursor()
        command = "select stud_gender,stud_dob, science, humanities, commerce, aptitude,extra from student_profile where stud_id =%s "
        ...
        def ages(birthdate):
            ...
            return age
        cursor.close()
        gender = 0 if res[0][0] =="Female" else 1
        age = ages(res[0][1])
        opens = int(res[0][2])
        neu = int(res[0][3])
        ext = int(res[0][4])
        ...
        model =train_model()
        model.train()
        opensss=int(opens)*10
        print(opensss)
        personality = model.test((gender,age, opens,neu,ext, nev,helps))
        labelss = ['Openess', 'neuroticism', 'conscientiousness', 'agreeableness','extraversion']

        valuess = [opens, neu, ext, nev,helps]

        colors = ["#F7464A", "#46BFBD", "#FDB45C", "#FEDCBA","#ABCDEF"]
        
        return render_template('prediction.html',person=personality,title='Bitcoin Monthly Price in USD', max=10,labels=labelss, values=valuess)

@app.route("/cvreport.html")
def cvreport():
    connection.ping(reconnect=True)
    cursor = connection.cursor()
    command="select Pathdirs from user_data where stud_id=%s "
    cursor.execute(command,session['id'])
    res=cursor.fetchone()
    print("hereeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee",res)
    paths=""
    for i in res:
        paths=i
    print("pathsssssssssssssssssssssssssssssssssssssssssssssssss",paths)




    save_image_path = paths
...
def reportview():
    ...
    marks5 = session['extra']
    percentage1 = mark1 
    percentage2 = mark2 
    percentage3 = mark3 
    percentage4 = mark4 
    percentage5 = (mark1 + mark2 + mark3 + mark4)
    query = "update student_profile set science = %s , commerce = %s, humanities = %s, aptitude = %s, extra = %s, total =%s where stud_id = %s"
    cursor.execute(query, (percentage1, percentage2, percentage3, percentage4, marks5, percentage5, session['id']))
    connection.ping(reconnect=True)
    connection.commit()
    cursor.close()
...
def questionweightageedit(id):
    connection.ping(reconnect=True)
    cursor = connection.cursor()
    command = "select question_id,science,commerce,humanities,apt,extra from question_paper where question_id =%s and question_paper_id =%s"
...
def addquestionweight():
    if request.method == 'POST':
        ...
        ext = request.form['des5']
        connection.ping(reconnect=True)
        cursor = connection.cursor()
        command = "update question_paper set science = '" + sci + "', commerce= '" + comm + "', humanities= '" + humani + "', apt='" + apt + "', extra='" + ext + "' where question_id = '" + qid + "' and question_paper_id = '" + \
                  session['quesid'] + "'"
        cursor.execute(command)
        connection.commit()
...
def post_question():
    if request.method == 'POST':
        ...
        if correct_answer == user_answer:
            ...
            session['extra'] = session['extra'] +  ext
        ...
        if result == 0:
            ...
            mark4 = session['aptitude'] 
            marks5 = session['extra']
            query = "update student_profile set science = %s , commerce = %s, humanities = %s, aptitude = %s, extra = %s where stud_id = %s"
            cursor.execute(query, (mark1, mark2, mark3, mark4, marks5,session['id']))
            connection.commit()
            cursor.close()
            flash(
...
def post_profile():
    ...
    if request.method == 'POST':
        ...
        if cv and allowed_file(cv.filename):
            ...
            print("fileeeeeeeeeeeeeenameeeeeeeeeeeeeeeeeeeeeeeeeeeee",filename)
            ...
            if resume_data:
                resume_text = pdf_reader(save_image_path)
                insert_data(resume_data['name'], resume_data['email'],resume_data['no_of_pages'], str(resume_data['skills']), session['id'],save_image_path)
                connection.commit()
...
def insert_data(name,email,no_of_pages,skills, stud_id,img_path):
    connection.ping(reconnect=True)
    cursor = connection.cursor()
    DB_table_name = 'user_data'
    insert_sql = "replace into user_data (Name,Email_ID,Page_no,Actual_skills,stud_id,Pathdirs) values (%s,%s,%s,%s,%s,%s)"
    rec_values = (name, email, no_of_pages, skills,stud_id,img_path)
